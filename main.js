...
for (let combo of dados) {
  ...
  cardCombo.className='min-w-[90%] md:min-w-[400px] bg-zinc-900 rounded-3xl p-2 flex relative overflow-hidden shadow-lg border border-yellow-400/20 snap-center'
  // Usando template string sem espaços extras nas propriedades
  const imagem = 'img1.jpg'
  
  
cardCombo.style.backgroundImage = `url('./img/${imagem}')`;
cardCombo.style.backgroundSize = "cover";
cardCombo.style.backgroundPosition = "center";
  cardCombo.innerHTML=`

        <div class="absolute inset-0 bg-gradient-to-r from-zinc-900/95 via-zinc-900/95 to-transparent"></div>
